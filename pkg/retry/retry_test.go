...
func TestRetrier_Do(t *testing.T) {
	...
	t.Run("exhausted retries", func(t *testing.T) {
		...
		if attempts != 2 {
			t.Errorf("Expected 2 actual attempts, got: %d", attempts)
		}
	})
...
func TestRetrier_Concurrent(t *testing.T) {
	...
	// 5 successful (1 attempt each) + 5 failed (2 attempts each) = 15
	if attempts != 15 {
		t.Errorf("Expected 15 total attempts, got: %d", attempts)
	}
}

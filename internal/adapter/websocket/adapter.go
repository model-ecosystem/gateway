...
func (a *Adapter) Start(ctx context.Context) error {
	...
	if a.config.TLS != nil && a.config.TLS.Enabled {
		...
		a.listener, err = tls.Listen("tcp", addr, tlsConfig)
	...
	if err != nil {
		return errors.NewError(errors.ErrorTypeInternal, "failed to create WebSocket listener").WithCause(err)
	}
...
func (a *Adapter) handleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	wsConn := newConn(conn, r.RemoteAddr)
	...
	if resp != nil && resp.StatusCode() == http.StatusSwitchingProtocols {
		a.logger.Debug("WebSocket connection handled", "path", r.URL.Path)
	}
}
